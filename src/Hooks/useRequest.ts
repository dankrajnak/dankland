...
import createIgnorablePromise from "../Utils/IgnorablePromise";
...
export type useRequestActions<T> = {
   ...
   */
  edit: (data: EditDataPayload<T>) => void;
};
...
function useRequest<T, P extends Array<unknown>>(
  ...
  const safeConfig: Config<T> = useMemo(
    () => ({
      ...
      genericErrorMessage: config?.genericErrorMessage,
    }),
    [config?.genericErrorMessage, config?.initialData]
  );
